...
Handles route analysis dashboard and data endpoints
"""
...
def get_route_analysis_data():
    ...
    try:
        routes = RouteAnalysis.query.all()
        ...
        # Find top route
        top_route = max(routes, key=lambda x: x.total_passengers)
...
def get_top_destinations():
    ...
    try:
        ...
        routes = RouteAnalysis.query.order_by(RouteAnalysis.total_passengers.desc()).limit(limit).all()
...
def get_growth_rates():
    ...
    try:
        ...
        routes = RouteAnalysis.query.filter(RouteAnalysis.previous_week > 0).order_by(RouteAnalysis.variance_pct.desc()).limit(limit).all()
...
def get_passenger_distribution():
    ...
    try:
        ...
        top_routes = RouteAnalysis.query.order_by(RouteAnalysis.total_passengers.desc()).limit(8).all()
        all_routes = RouteAnalysis.query.all()
...
def get_week_comparison():
    ...
    try:
        ...
        routes = RouteAnalysis.query.order_by(RouteAnalysis.total_passengers.desc()).limit(10).all()
