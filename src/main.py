...
from src.routes.manifest import manifest_bp  # NEW: Manifest system

app = Flask(__name__, static_folder=os.path.join(os.path.dirname(__file__), 'static'))
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev-secret-key-change-in-production')
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024  # Increased to 50MB for manifest files
...
app.register_blueprint(manifest_bp, url_prefix='/api/manifest')  # NEW: Manifest API routes
...
@app.route('/flight-load/manifest')
def flight_load_manifest():
    """NEW: Serve the manifest dashboard"""
    static_folder_path = app.static_folder
    if static_folder_path is None:
        return "Static folder not configured", 404
